...
export default function MaterialsPage({ params }:{ params:{ lang: Locale }}) {
  ...
  const { data: materials = [], mutate: refetch } = useSWR<Material[]>('/api/materials', fetcher);
  ...
  return (
    <div className="chrome">
      ...
      <MaterialModal
        ...
        onSaved={()=>{refetch(); if(selectedMaterial) setSelectedMaterial(materials.find(m => m.id === selectedMaterial.id) || null)}}
        onCreated={()=>refetch()}
      />
    </div>
  );
