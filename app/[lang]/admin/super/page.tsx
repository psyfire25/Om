...
export default function Super({ params }:{ params:{ lang: Locale } }){
  ...
  async function createInvite(e:React.FormEvent<HTMLFormElement>){
    ...
    if (!res.ok) {
      alert('Failed to create invite: ' + (await res.text()));
      return;
    }
  ...
  return (
    <div className="chrome">
      ...
      <div className="columns">
          ...
          <Accordion title={t(lang,'usersInvites')} defaultOpen>
            {me?.role!=='SUPER' ? <p>Forbidden</p> : (
              <>
                ...
                <table>
                  <thead>
                    <tr><th>Email</th><th>Role</th><th>Expires</th><th>Used</th></tr>
                  </thead>
                  <tbody>
                    {invites.map((i:any)=>(
                      <tr
                        key={i.token}
                        className="clickable"
                        onClick={()=>setInviteToken(i.token)}
                        title="Open invite"
                      >
                        <td>{i.email||'—'}</td>
                        <td>{i.role}</td>
        ...
        <div className="column">
          <div className="card">
            <p>Generate an invite, then click a row to edit details, extend expiry, copy link, or revoke.</p>
          </div>
        </div>
      </div>
