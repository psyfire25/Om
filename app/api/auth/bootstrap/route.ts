...
export async function POST() {
  if (db.data.users.find(u=>u.role==='SUPER')) return new NextResponse('Already bootstrapped', { status: 400 });
  ...
  const user = { id: crypto.randomUUID(), name, email, role: 'SUPER', passwordHash, active: true, createdAt: now, updatedAt: now };
  db.data.users.push(user as any); db.write();
